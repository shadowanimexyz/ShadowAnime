const API = "https://anikoto-i4xchl96t-shadowanimexyzs-projects.vercel.app";
...
if(document.getElementById("homeAnimeCount")){
  fetch(`${API}/api/latest-episodes?page=1`)
    .then(res => res.json())
    .then(json => {
  const data = Array.isArray(json)
    ? json
    : json.results?.data || json.results || json.data || [];

  homeAnimeCount.innerText = data.length;
})
...
if(typeof renderManga === "function"){
  renderManga();
}
...
if(videoPlayer){
    ...
    history.replaceState(
        null,
        "",
        `anime-details.html?id=${animeIndex}&ep=${currentEp}`
    );
}
...
function loadAutoHomeAnime(){
  ...
  fetch(`${API}/api/latest-episodes?page=1`)
    .then(res => res.json())
    .then(json => {
      console.log("HOME API DATA:", json);
      ...
      if(!data.length){
        grid.innerHTML = "<h2>No anime found from API</h2>";
        return;
      }
      ...
      function showRandomAnime(){
        ...
        [...data].sort(() => Math.random() - 0.5).slice(0, 6).forEach(anime => {
          grid.innerHTML += `
            <div class="shadow-card auto-card">
              <img src="${anime.poster || anime.image || anime.thumbnail || 'image/default.jpg'}">
              <div class="shadow-info">
                <h3>${anime.title || anime.name || 'Unknown Anime'}</h3>
                <p>${anime.episodes || anime.quality || "Anime"}</p>
                <a href="anime-details.html?id=${anime.id || anime.slug || anime.title}">
                  Watch Now
                </a>
              </div>
            </div>
      ...
      showRandomAnime();
      setInterval(showRandomAnime, 4000);
    })
    .catch(err => {
      console.error("Home API Error:", err);
      grid.innerHTML = "<h2>API failed / CORS issue</h2>";
    });
}
loadLiveCounts();
loadAutoHomeAnime();
...
animeSearch.addEventListener("input", () => {
  clearSearch.style.display = animeSearch.value.trim() ? "block" : "none";
});
