...
function renderHomeAnime(search = ""){

    if(!homeAnimeGrid) return;
    ...
    if(keyword === ""){
        filteredAnime = animeData.slice(0, 8);
    }

    homeAnimeGrid.innerHTML = filteredAnime.map(anime => `
        ...
        <div class="card">
            ...
            <div class="card-overlay">
                ...
                <p>
                    ${anime.category || anime.genre || "Anime"}
                    • ⭐ ${anime.rating || "N/A"}
                </p>
    ...
    if(filteredAnime.length === 0){
        homeAnimeGrid.innerHTML = `
            <div class="empty-box">
                <h2>No anime found</h2>
                <p>Try another name, genre, studio, or year.</p>
            </div>
        `;
    }
}
...
if(searchInput){
    searchInput.addEventListener("input", e => {
        renderHomeAnime(e.target.value);
    });
}
...
if(document.getElementById("homeAnimeCount")){
    homeAnimeCount.innerText = animeData.length;
}

if(document.getElementById("homeMovieCount")){
    homeMovieCount.innerText = movieData.length;
}
...
const mangaData = [

    {
        ...
        rating:"9.9",
        chapters:"1100+",
        desc:"Legendary pirate adventure manga."
    },

    {
        ...
        rating:"9.8",
        chapters:"370+",
        desc:"Dark medieval masterpiece manga."
    },

    {
        name:"Blue Lock",
        genre:"Sports",
        image:"image/bluelock.jpg",
        rating:"9.2",
        chapters:"250+",
        desc:"Football ego survival manga."
    },

    {
        ...
        rating:"9.5",
        chapters:"260+",
        desc:"Curse battle supernatural manga."
    }

];

function renderManga(search = ""){
    ...
    let filter =
    mangaFilter ? mangaFilter.value.toLowerCase() : "all";

    let filteredManga = mangaData.filter(manga => {
        ...
        let matchFilter =
        filter === "all" ||
        manga.genre.toLowerCase().includes(filter);
